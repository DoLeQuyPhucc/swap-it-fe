...
const PackagesScreen = () => {
  ...
  const handlePayment = async (packageDetails: any) => {
    ...
    try {
      const paymentData = {
        orderCode: Math.floor(Math.random() * 1000000), // Tạo số ngẫu nhiên làm mã đơn hàng
        amount: parseFloat(packageDetails.price.replace(/[^0-9]/g, '')),
        description: `Payment for ${packageDetails.title}`.slice(0, 25), // Cắt chuỗi để đảm bảo không quá 25 ký tự
